...
const CHECKLIST_ROWS = [
  { key: "liveDemoChecked" as const, label: "Live demo checked" },
  { key: "repositoryPublic" as const, label: "Repository publicly accessible" },
  { key: "documentationComplete" as const, label: "Documentation complete" },
  { key: "testsVerified" as const, label: "Tests verified" },
  { key: "licenseClear" as const, label: "License clear" },
];
          ...
          <div className="frosted-panel">
            <header className="border-b border-white/10 pb-8">
              ...
              <p className="mt-3 text-base leading-relaxed text-neutral-300">
                ...
                {" · "}
                Issued by <span className="font-medium text-white">{proof.issuerName}</span>
              </p>
              <dl className="mt-3 grid gap-1 text-sm text-neutral-300 sm:grid-cols-2">
                <div className="w-fit rounded bg-white/10 px-1.5 py-0.5 font-mono">
                  <dt className="inline font-extrabold text-emerald-300">Proof ID</dt>
                  <dd className="inline font-mono text-emerald-300"> {proof.id}</dd>
                </div>
                <div className="w-fit rounded bg-white/10 px-1.5 py-0.5 font-mono">
                  <dt className="inline font-extrabold text-emerald-300">Proof created</dt>
                  <dd className="inline text-emerald-300"> {formatDate(proof.createdAt)}</dd>
                </div>
              </dl>
            </header>

            <section className="py-3" aria-labelledby="projects-heading">
              <h2 id="projects-heading" className="text-xl font-semibold tracking-tight text-white">
                Projects
              </h2>
              ...
              {projects.length === 0 ? (
                ...
                >
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-white/80">
                    This proof has no projects attached. Add some from the create proof page.
                  </p>
                </div>
              ) : (
            ...
            <footer className="border-t border-white/10 pt-3 text-sm text-neutral-300">
              ...
              <p className="mt-1 text-neutral-400">Generated via Git Proof (demo)</p>
              <p className="mt-1 w-fit max-w-full rounded bg-white/10 px-1.5 py-0.5 font-mono text-xs text-emerald-300">
  ...
  return (
    <article className="flex h-[300px] w-full flex-col overflow-y-auto rounded-xl border border-white/20 bg-green-300/30 p-3.5 shadow-sm backdrop-blur-xs md:p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <h3 className="text-base font-semibold leading-tight text-white">{project.name}</h3>
        <p
          className={
        ...
        >
          ...
          {project.status === "verified" ? "Verified" : "Revoked"}
        </p>
      </div>

      <p className="mt-2 line-clamp-2 text-sm leading-snug text-white/85">{project.description}</p>

      <p className="mt-1.5 text-[11px] leading-snug text-white/55">
        <span className="font-medium text-white/75">Issued</span> {formatDate(project.issuedAt)}
      </p>

      <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-sm">
        <a href={project.repoUrl} className={linkClass} rel="noopener noreferrer" target="_blank">
          Repository
        </a>
        {project.liveUrl ? (
          <a href={project.liveUrl} className={linkClass} rel="noopener noreferrer" target="_blank">
            Live demo
          </a>
        ) : null}
      </div>
      ...
      >
        <h4 className="text-xs font-semibold text-white">Checklist</h4>
            ...
            return (
              <li key={key} className="flex gap-2">
                ...
                <span className="min-w-0 text-white/90">
                  {label}
                  <span className="sr-only">{checked ? ", confirmed" : ", not confirmed"}</span>
                </span>
              </li>
            );
          })}
